...
def query_data(event=None):
    try:
        ...
        if row:
            ...
            label_description.config(text=f"Description: {row[3]}", fg="green")  # Màu xanh lá cho mô tả
...
label_search = Label(root, text="Enter Music ID or Name:", fg="purple")  # Màu tím cho nhãn
...
label_name = Label(root, text="Name: ", fg="blue")  # Màu xanh cho tên
label_name.pack(pady=5)
label_description = Label(root, text="Description: ", fg="green")  # Màu xanh lá cho mô tả
label_description.pack(pady=5)

# Nút truy vấn để phát nhạc
button_query = Button(root, text="Truy vấn và phát nhạc", command=query_data, bg="lightblue", fg="black")  # Nút màu xanh nhạt
button_query.pack(pady=20)

# Nút dừng nhạc
button_stop = Button(root, text="Dừng nhạc", command=stop_music, bg="red", fg="white")  # Nút dừng nhạc màu đỏ
