...
def get_music_list():
    try:
        ...
        for row in rows:
            music_listbox.insert(END, f"{row[0]} - {row[1]} by {row[2]} ({row[3]})")
    except Exception as e:
...
# Truy vấn danh sách video từ bảng
def get_movie_list():
    try:
        conn = connect_to_database()
        cursor = conn.cursor()
        ...
        for row in rows:
            movie_listbox.insert(END, f"{row[0]} - {row[1]} ({row[2]})")
    except Exception as e:
...
# Gọi hàm để lấy danh sách nhạc và video ngay khi khởi động
get_music_list()
get_movie_list()
